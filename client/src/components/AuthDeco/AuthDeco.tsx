...
const NAMES = ["chart", "stat", "metrics", "loader", "linechart", "tags", "heatmap", "notification"];
...
const AuthDeco = () => {
  ...
  useEffect(() => {
    const update = () => {
      if (sceneRef.current) {
        setSize({ w: sceneRef.current.offsetWidth, h: sceneRef.current.offsetHeight });
      }
    };
    update();
  ...
  const getStyle = (idx: number): React.CSSProperties => {
    ...
    const base = { "--tx": `${tx}px`, "--ty": `${ty}px` } as React.CSSProperties;

    const floatAnims = ["float-a", "float-b", "float-a", "float-c", "float-b", "float-a", "float-c", "float-b"];
    ...
    if (phase === "expand") {
      return { ...base, animation: `exp-card 1.1s cubic-bezier(0.16,1,0.3,1) ${idx * 0.09}s forwards` };
    }
    if (phase === "collapse" && expanded) {
      return { ...base, animation: `col-card 0.9s cubic-bezier(0.16,1,0.3,1) ${idx * 0.07}s forwards` };
    }
    if (expanded) {
      // Карточка на месте — добавляем float
      return { ...base, opacity: 1, transform: `translate(calc(-50% + ${tx}px), calc(-50% + ${ty}px)) scale(1)`, animation: floatAnim };
    }
    return base;
  };

  const extra = (name: string, idx: number) =>
    `auth-deco__card auth-deco__card--${name} auth-deco__card--extra`;
    ...
    <div className="auth-deco">
      ...
      <div className={`auth-deco__scene`} ref={sceneRef}>
        ...
        >
          ...
          <div className="auth-deco__rows">
            {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
              <div key={i} className="auth-deco__row" style={{ "--i": i } as React.CSSProperties}>
                <div className={`auth-deco__check-box${i < 4 ? " auth-deco__check-box--done" : ""}`}>
                  {i < 4 && (
                ...
                <div className="auth-deco__row-bars">
                  <div className="auth-deco__bar" style={{ width: `${48 + i * 6}%`, opacity: i < 4 ? 0.38 : 1 } as React.CSSProperties} />
                  <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: `${28 + i * 5}%` } as React.CSSProperties} />
                </div>
                <div className={`auth-deco__badge auth-deco__badge--${i % 3}`} />
              </div>
            ))}
          </div>
          ...
          <div className="auth-deco__progress-section">
            ...
            <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: 44 }} />
          </div>
        ...
        <div className={extra("chart", 0)} style={getStyle(0)}>
            <div className="auth-deco__card-header">
              <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "55%" }} />
            </div>
            <div className="auth-deco__chart">
              {[40, 65, 50, 80, 55, 90, 70, 60].map((h, i) => (
                <div key={i} className="auth-deco__chart-bar" style={{ "--h": `${h}%`, "--i": i } as React.CSSProperties} />
              ))}
            </div>
        </div>
        ...
        <div className={extra("stat", 1)} style={getStyle(1)}>
            <div className="auth-deco__stat-circles">
              {[0, 1, 2].map((i) => (
                <div key={i} className={`auth-deco__stat-circle auth-deco__stat-circle--${i}`} />
              ))}
            </div>
            <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "65%" }} />
        </div>

        {/* ── Метрики ── */}
        <div className={extra("metrics", 2)} style={getStyle(2)}>
            <div className="auth-deco__metrics">
              {[["24", "задачи"], ["87%", "готово"], ["3", "просроч."]].map(([val, lbl], i) => (
        ...
        <div className={extra("loader", 3)} style={getStyle(3)}>
            ...
            <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "60%" }} />
        </div>

        {/* ── Линейный график ── */}
        <div className={extra("linechart", 4)} style={getStyle(4)}>
            <div className="auth-deco__card-header">
              <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "45%" }} />
            </div>
            <svg className="auth-deco__line-svg" viewBox="0 0 120 40" preserveAspectRatio="none">
              <polyline className="auth-deco__line-track" points="0,35 20,28 40,32 60,18 80,22 100,10 120,14" />
              <polyline className="auth-deco__line-fill" points="0,35 20,28 40,32 60,18 80,22 100,10 120,14 120,40 0,40" />
            </svg>
        </div>
        ...
        <div className={extra("tags", 5)} style={getStyle(5)}>
            <div className="auth-deco__tags">
              {["Работа", "Личное", "Срочно", "Идеи", "Обучение"].map((t, i) => (
                <span key={t} className={`auth-deco__tag auth-deco__tag--${i % 4}`}>{t}</span>
              ))}
            </div>
        </div>
        ...
        <div className={extra("heatmap", 6)} style={getStyle(6)}>
            <div className="auth-deco__card-header">
              <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "50%" }} />
            </div>
            <div className="auth-deco__heatmap">
              {Array.from({ length: 35 }).map((_, i) => (
                <div key={i} className="auth-deco__heatmap-cell" style={{ "--level": i % 4 } as React.CSSProperties} />
              ))}
            </div>
        </div>
        ...
        <div className={extra("notification", 7)} style={getStyle(7)}>
            <div className="auth-deco__notification">
              <div className="auth-deco__dot auth-deco__dot--accent" style={{ flexShrink: 0 }} />
              <div className="auth-deco__row-bars">
                <div className="auth-deco__bar auth-deco__bar--short" />
                <div className="auth-deco__bar auth-deco__bar--xs" style={{ width: "70%" }} />
              </div>
            </div>
        </div>

        {[0, 1, 2, 3, 4, 5, 6].map((i) => (
          <div key={i} className={`auth-deco__particle auth-deco__particle--${i}`} />
        ))}
      </div>
    </div>
  );
