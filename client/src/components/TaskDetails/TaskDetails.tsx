...
const TaskDetails = ({ task, onClose }: TaskDetailsProps) => {
  ...
  if (!task || !draft) {
    return (
      <aside className="task-details task-details--empty">
  ...
  const addCheckListItem = (e: React.FormEvent) => {
    e.preventDefault();
    if (!newCheckLinkTitle.trim()) return;
          ...
          <div className="task-meta-grid">
            ...
            >
              ...
              <div
                ...
                style={{
                  color: getPriorityMeta(currentData.priority as Priority)
                    .color,
                }}
              >
                <Flag size={14} />
          ...
          {isEditing ? (
            ...
            <div className="task-details__notes-display">
              {task.notes ? (
                ...
                <span className="task-details__empty-text">Нет описания</span>
              )}
            </div>
          )}
        </div>
        ...
        <div className="task-details__section task-details__section--checklist">
          <div className="task-details__section-header">
            ...
            <span className="task-checklist-count">
              {(currentData.checkList || []).filter((i) => i.completed).length}{" "}
              / {(currentData.checkList || []).length}
            </span>
          </div>

          <div className="task-checklist-items">
            {(currentData.checkList || []).length === 0 && !isEditing && (
              <span className="task-details__empty-text">Чек-лист пуст</span>
            )}
          ...
          {isEditing && (
            <form className="task-checklist-add" onSubmit={addCheckListItem}>
              <Plus size={18} />
              <input
                type="text"
              ...
              />
              {newCheckLinkTitle && <button type="submit">Добавить</button>}
            </form>
          )}
        </div>
      </div>
